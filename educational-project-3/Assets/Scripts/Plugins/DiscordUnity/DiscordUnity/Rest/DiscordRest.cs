...
namespace Plugins.DiscordUnity.DiscordUnity.Rest
{
    public static partial class DiscordAPI
    {
        ...
        private static async Task<RestResult<R>> SyncInherit<T, R>(Task<RestResult<T>> call, Func<T, R> transform)
        {
            ...
            if (result) DiscordUnity.DiscordAPI.Sync(() => task.SetResult(RestResult<R>.FromResult(transform(result.Data))));
            else DiscordUnity.DiscordAPI.Sync(() => task.SetResult(RestResult<R>.FromException(result.Exception)));
